...
@NotNullByDefault
final class TarArkivoPath implements Path {
    /// The TAR file system that owns this path.
    private final TarArkivoFileSystemImpl fileSystem;

    /// Whether this path starts at the TAR file system root.
    ...
    /// Returns a TAR path from a path text string.
    private static TarArkivoPath parse(TarArkivoFileSystemImpl fileSystem, String value) {
        ...
        return new TarArkivoPath(fileSystem, absolute, names);
    }
    ...
    static TarArkivoPath require(Path path, TarArkivoFileSystemImpl expectedFileSystem) {
        if (path instanceof TarArkivoPath tarPath && tarPath.fileSystem == expectedFileSystem) {
            return tarPath;
        }
    ...
    /// Returns the owner TAR file system.
    @Override
    public TarArkivoFileSystemImpl getFileSystem() {
        return fileSystem;
    }

    /// Returns whether this path starts at the TAR file system root.
    @Override
    ...
    public @Nullable Path getFileName() {
        ...
        return new TarArkivoPath(fileSystem, false, List.of(names.get(names.size() - 1)));
    }
    ...
    public @Nullable Path getParent() {
        ...
        return new TarArkivoPath(fileSystem, absolute, names.subList(0, names.size() - 1));
    }
    ...
    public Path getName(int index) {
        return new TarArkivoPath(fileSystem, false, List.of(names.get(index)));
    }

    /// Returns a relative path containing a range of name elements.
    @Override
    public Path subpath(int beginIndex, int endIndex) {
        return new TarArkivoPath(fileSystem, false, names.subList(beginIndex, endIndex));
    }

    /// Returns whether this path starts with another path.
    @Override
    public boolean startsWith(Path other) {
        TarArkivoPath that = asTarPath(other);
    ...
    public Path normalize() {
        ...
        return new TarArkivoPath(fileSystem, absolute, normalizedNames);
    }

    /// Resolves another path against this path.
    @Override
    public Path resolve(Path other) {
        TarArkivoPath that = requireTarPath(other);
        if (that.absolute) {
            return that;
        }
        ...
        return new TarArkivoPath(fileSystem, absolute, resolvedNames);
    }
    ...
    public Path relativize(Path other) {
        ...
        return new TarArkivoPath(fileSystem, false, relativeNames);
    }
    ...
    public Iterator<Path> iterator() {
        ...
        for (String name : names) {
            paths.add(new TarArkivoPath(fileSystem, false, List.of(name)));
        }
    ...
    public boolean equals(@Nullable Object object) {
        return object instanceof TarArkivoPath that
    ...
    /// Returns this path as TAR entry path text.
    @Override
    public String toString() {
        return text;
    }

    /// Returns this path as a TAR path from the same file system, or `null` when it is incompatible.
    private @Nullable TarArkivoPath asTarPath(Path path) {
        if (path instanceof TarArkivoPath that && that.fileSystem == fileSystem) {
            return that;
        }
        return null;
    }

    /// Returns this path as a TAR path from the same file system.
    private TarArkivoPath requireTarPath(Path path) {
        TarArkivoPath that = asTarPath(path);
        if (that == null) {
    ...
    /// Returns an encoded URI path for TAR path names.
    ...
    private static String uriPathSegment(String name) {
        try {
            ...
            throw new IllegalArgumentException("Invalid TAR path segment", exception);
        }
    }

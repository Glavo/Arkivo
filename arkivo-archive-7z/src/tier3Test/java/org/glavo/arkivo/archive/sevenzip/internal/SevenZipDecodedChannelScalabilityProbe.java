...
@NotNullByDefault
public final class SevenZipDecodedChannelScalabilityProbe {
    ...
    private SevenZipDecodedChannelScalabilityProbe() {
    }
    ...
    /// Creates a highly compressible 7z entry without retaining its decoded body in memory.
    private static void create(Path archive) throws IOException {
        Files.createDirectories(archive.toAbsolutePath().getParent());
        byte @Unmodifiable [] block = expectedBlock();
    ...
    private static void verifyRange(SeekableByteChannel channel, long position, int length) throws IOException {
        ...
        byte @Unmodifiable [] actual = bytes.array();
    ...
    private static byte @Unmodifiable [] expectedBlock() {

...
@NotNullByDefault
final class SevenZipArkivoPath implements Path {
    /// The 7z file system that owns this path.
    private final SevenZipArkivoFileSystem fileSystem;

    /// Whether this path starts at the 7z file system root.
    ...
    /// Returns a 7z path from a path text string.
    private static SevenZipArkivoPath parse(SevenZipArkivoFileSystem fileSystem, String value) {
        ...
        return new SevenZipArkivoPath(fileSystem, absolute, names);
    }

    /// Returns the owner 7z file system.
    @Override
    public SevenZipArkivoFileSystem getFileSystem() {
        return fileSystem;
    }

    /// Returns whether this path starts at the 7z file system root.
    @Override
    ...
    public @Nullable Path getFileName() {
        ...
        return new SevenZipArkivoPath(fileSystem, false, List.of(names.get(names.size() - 1)));
    }
    ...
    public @Nullable Path getParent() {
        ...
        return new SevenZipArkivoPath(fileSystem, absolute, names.subList(0, names.size() - 1));
    }
    ...
    public Path getName(int index) {
        return new SevenZipArkivoPath(fileSystem, false, List.of(names.get(index)));
    }

    /// Returns a relative path containing a range of name elements.
    @Override
    public Path subpath(int beginIndex, int endIndex) {
        return new SevenZipArkivoPath(fileSystem, false, names.subList(beginIndex, endIndex));
    }

    /// Returns whether this path starts with another path.
    @Override
    public boolean startsWith(Path other) {
        SevenZipArkivoPath that = asSevenZipPath(other);
    ...
    public Path normalize() {
        ...
        return new SevenZipArkivoPath(fileSystem, absolute, normalizedNames);
    }

    /// Resolves another path against this path.
    @Override
    public Path resolve(Path other) {
        SevenZipArkivoPath that = requireSevenZipPath(other);
        if (that.absolute) {
            return that;
        }
        ...
        return new SevenZipArkivoPath(fileSystem, absolute, resolvedNames);
    }
    ...
    public Path relativize(Path other) {
        ...
        return new SevenZipArkivoPath(fileSystem, false, relativeNames);
    }

    /// Converts this path to a URI.
    @Override
    public URI toUri() {
        SevenZipArkivoPath absolutePath = (SevenZipArkivoPath) toAbsolutePath().normalize();
        return URI.create(
                SevenZipArkivoFileSystemProvider.SCHEME
                        + ":"
    ...
    public Iterator<Path> iterator() {
        ...
        for (String name : names) {
            paths.add(new SevenZipArkivoPath(fileSystem, false, List.of(name)));
        }
    ...
    public boolean equals(@Nullable Object object) {
        return object instanceof SevenZipArkivoPath that
    ...
    /// Returns this path as 7z entry path text.
    @Override
    public String toString() {
        return text;
    }

    /// Returns this path as a 7z path from the same file system, or `null` when it is incompatible.
    private @Nullable SevenZipArkivoPath asSevenZipPath(Path path) {
        if (path instanceof SevenZipArkivoPath that && that.fileSystem == fileSystem) {
            return that;
        }
        return null;
    }

    /// Returns this path as a 7z path from the same file system.
    private SevenZipArkivoPath requireSevenZipPath(Path path) {
        SevenZipArkivoPath that = asSevenZipPath(path);
        if (that == null) {
    ...
    private URI archiveUri() {
        ...
        throw new UnsupportedOperationException("7z paths backed by volume sources cannot be converted to URIs");
    }

    /// Returns an encoded URI path for 7z path names.
    ...
    private static String uriPathSegment(String name) {
        try {
            ...
            throw new IllegalArgumentException("Invalid 7z path segment", exception);
        }
    }

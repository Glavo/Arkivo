...
/// Incrementally encodes one zlib stream without binding codec state to an output channel.
@NotNullByDefault
public final class ZlibEncoder implements CompressionEncoder {
    ...
    /// JDK zlib-wrapped Deflate context.
    ...
    /// Sync-flush bytes waiting to be copied into caller-owned target buffers.
    ...
    public ZlibEncoder(
        ...
        this.dictionary = dictionary != null ? validatedDictionaryBytes(dictionary) : null;
    ...
    /// Finishes the zlib stream without releasing the native context.
    @Override
    ...
    /// Applies immutable configuration to a fresh JDK zlib context session.
    ...
    /// Returns dictionary bytes after validating a known Adler-32 identifier.
    private static byte[] validatedDictionaryBytes(CompressionDictionary dictionary) {

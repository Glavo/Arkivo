...
final class DirectByteBufferCodecSupport {
    ...
    private static ByteBuffer decodeAllocating(
        ...
        CompressionCodec<?> engineCodec =
                codec.withMaximumOutputSize(CompressionCodec.UNLIMITED_SIZE);

        try (CompressionDecoder decoder = engineCodec.newDecoder()) {
    ...
    private static void decode(
        ...
        try (CompressionDecoder decoder = engineCodec.newDecoder()) {
            ...
            while (true) {
                ...
                try {
                    outcome = decoder.finish(source, operationTarget);
                } finally {

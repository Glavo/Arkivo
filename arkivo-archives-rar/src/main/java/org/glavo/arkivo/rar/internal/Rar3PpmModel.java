...
/// Decodes the PPMd Variant H context model used by RAR3 compressed blocks.
@NotNullByDefault
final class Rar3PpmModel {
    ...
    private final Rar3PpmAllocator allocator = new Rar3PpmAllocator();
    /// The arithmetic range decoder.
    private final Rar3PpmRangeDecoder rangeDecoder = new Rar3PpmRangeDecoder();
    ...
    void initialize(
            ...
            int maximumMebibytes
    ) throws IOException {
        Objects.requireNonNull(input, "input");
        rangeDecoder.initialize(input);
        if (!reset) {
            if (!initialized) throw new IOException("RAR3 PPM continuation has no initialized model");
            return;
        }
        if (requestedMaximumOrder == 1) throw new IOException("RAR3 PPM maximum order one is invalid");
        allocator.initialize(maximumMebibytes);
    ...
    int readByte() throws IOException {
        if (!initialized) throw new IOException("RAR3 PPM model is not initialized");
        ...
        while (selectedState == NO_STATE) {
            ...
            do {
                ...
                if (minimumContext == 0) throw new IOException("Corrupt RAR3 PPM suffix chain");
    ...
    private void restartModel() throws IOException {
        ...
        if (currentContext == 0) throw new IOException("Unable to allocate the RAR3 PPM root context");
    ...
    private int decodeBinarySymbol(int context) throws IOException {
        ...
        if (suffix == 0) throw new IOException("Corrupt RAR3 PPM binary context");
        ...
        if (frequency <= 0 || frequency > binaryProbabilities.length) {
            throw new IOException("Corrupt RAR3 PPM binary frequency");
        }
        int probability = binaryProbabilities[frequency - 1][contextClass];
        int mean = probability + (1 << PERIOD_BITS - 2) >> PERIOD_BITS;
        if (rangeDecoder.currentCount(BINARY_SCALE) < probability) {
            rangeDecoder.decode(0, probability);
        ...
        rangeDecoder.decode(probability, BINARY_SCALE);
        probability -= mean;
    ...
    private int decodeFirstOrderSymbol(int context) throws IOException {
        ...
        if (scale == 0) throw new IOException("Corrupt RAR3 PPM zero-frequency context");
    ...
    private int decodeEscapedSymbol(int context, int maskedStateCount) throws IOException {
        ...
        if (availableCount <= 0) throw new IOException("Corrupt RAR3 PPM exclusion state");
        ...
        for (int index = 0; index < availableCount; index++) {
            ...
            if (sourceOffset >= stateCount) throw new IOException("Corrupt RAR3 PPM symbol mask");
    ...
    private int createSuccessors(int context, int selectedState, int matchingSuffixState) throws IOException {
        ...
        while (allocator.contextSuffix(context) != 0) {
            ...
            if (buffered >= successorStateIndexes.length) {
                throw new IOException("RAR3 PPM successor depth exceeds the symbol alphabet");
            }
        ...
        if (context == 0 || allocator.successor(selectedState) >= 0) {
            throw new IOException("Corrupt RAR3 PPM successor history");
        }

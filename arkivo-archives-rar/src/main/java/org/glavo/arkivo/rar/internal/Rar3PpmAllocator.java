...
/// Stores RAR3 PPMd contexts in the format-defined split suballocator.
@NotNullByDefault
final class Rar3PpmAllocator {
    ...
    /// Allocates an arena with the requested integral number of mebibytes.
    void initialize(int maximumMebibytes) throws IOException {
        if (maximumMebibytes <= 0 || maximumMebibytes > 256) {
            throw new IOException("RAR3 PPM memory size must be between 1 and 256 MiB");
        }
        long byteCount = (long) maximumMebibytes << 20;
        ...
        try {
            ...
            throw new IOException("Unable to allocate the RAR3 PPM memory arena", error);
        }
    }

    /// Restarts allocations at the initial split-heap boundaries.
    void restart() throws IOException {
        if (symbols.length == 0) throw new IOException("RAR3 PPM allocator is not initialized");
    ...
    int findState(int context, int symbol) throws IOException {
        ...
        throw new IOException("RAR3 PPM context does not contain the required symbol");
    }

...
/// Tests native 7z Delta and BCJ filters against independent XZ filter streams.
@NotNullByDefault
public final class SevenZipNativeFilterTest {
    /// The nonzero start offset used to verify state initialization.
    private static final int START_OFFSET = 0x1000;

    /// Verifies Delta encoding and decoding at the minimum, ordinary, and maximum distances.
    @Test
    public void deltaInteroperabilityAcrossChunks() throws IOException {
        ...
        for (int distance : new int[]{1, 3, 256}) {
            byte[] expected = encodeWithXz(original, new DeltaOptions(distance));
            byte[] encoded = encodeNatively(original, new DeltaFilter(true, distance));
            assertArrayEquals(expected, encoded);
            assertArrayEquals(
                    original,
                    decodeNatively(encoded, new DeltaFilter(false, distance))
            );
        }
    }
    ...
    private static byte[] encodeNatively(byte[] original, ByteFilterTransform transform) throws IOException {
        ByteArrayOutputStream target = new ByteArrayOutputStream();
        try (ByteFilterOutputStream output = new ByteFilterOutputStream(target, transform)) {
    ...
    private static byte[] decodeNatively(byte[] encoded, ByteFilterTransform transform) throws IOException {
        try (ByteFilterInputStream input = new ByteFilterInputStream(
    ...
    private interface TransformFactory {
        ...
        ByteFilterTransform create(boolean encoder, int startOffset);
    }

...
public final class RarLowHeapStorageProbe {
    ...
    public static void main(String @Unmodifiable [] arguments) throws IOException {
    ...
    private static void createArchive(Path archivePath) throws IOException {
        byte @Unmodifiable [] zeros = new byte[BUFFER_SIZE];
        ...
        byte @Unmodifiable [] name = "large.bin".getBytes(StandardCharsets.US_ASCII);
    ...
    private static void writeBlockHeader(OutputStream output, int type, long flags, byte @Unmodifiable [] fields)
        ...
        header.write(fields);
        byte @Unmodifiable [] bytes = header.toByteArray();

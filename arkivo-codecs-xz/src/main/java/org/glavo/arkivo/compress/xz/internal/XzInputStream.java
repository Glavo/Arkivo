    ...
    private final class BlockInput extends InputStream {
        ...
        private InputStream openFilter(InputStream downstream, Filter filter) throws IOException {
            ...
            if (identifier == XzSupport.FILTER_DELTA) {
                ...
                return new ByteFilterInputStream(
                        downstream,
                        new DeltaFilter(false, Byte.toUnsignedInt(properties[0]) + 1)
                );
            }
            if (identifier >= XzSupport.FILTER_BCJ_X86 && identifier <= XzSupport.FILTER_BCJ_RISCV) {
                ...
                return new ByteFilterInputStream(downstream, bcjTransform(identifier, startOffset));
            }
            throw new IOException("Unsupported XZ filter identifier: " + identifier);
        }

        /// Creates one BCJ decoding transform.
        private ByteFilterTransform bcjTransform(long identifier, int startOffset) {
            return switch ((int) identifier) {
                case 0x04 -> BCJFilters.x86(false, startOffset);
                case 0x05 -> BCJFilters.powerPc(false, startOffset);
                case 0x06 -> BCJFilters.ia64(false, startOffset);
                case 0x07 -> BCJFilters.arm(false, startOffset);
                case 0x08 -> BCJFilters.armThumb(false, startOffset);
                case 0x09 -> BCJFilters.sparc(false, startOffset);
                case 0x0a -> BCJFilters.arm64(false, startOffset);
                case 0x0b -> BCJFilters.riscV(false, startOffset);
